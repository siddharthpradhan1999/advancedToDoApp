...
import { collection, addDoc, getDocs, where, query, deleteDoc, updateDoc, doc, setDoc, documentId } from "firebase/firestore";
...
export default function Home() {
    ...
    const addToDo = async () => {
        try {
            const docRef = await addDoc(collection(db, "advancedToDos"), {
                owner: authUser.uid,
                content: todoInput,
                completed: false
            });
    ...
    const fetchToDos = async (uid) => {
        try {
            const q = query(collection(db, "advancedToDos"), where("owner", "==", uid));
    ...
    const deleteTodo = async (docId) => {
        try {
            await deleteDoc(doc(db, "advancedToDos", docId))
            fetchToDos(authUser.uid)
        } catch (error) {
            console.log(error)
        }
    ...
    const markAsCompleteHandler = async (event, docId) => {
        try {
            const docRef = doc(db, "advancedToDos", docId)
